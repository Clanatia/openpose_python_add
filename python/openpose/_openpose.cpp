...
#define default_face_render_threshold 0.4f
...
DEFINE_string(face_net_resolution,	"368x368",	"Multiples of 16 and squared. Analogous to `net_resolution` but applied to the face keypoint"
												...
												" faces on the image.");
#define heatmaps_add_parts false
#define heatmaps_add_bkg false
#define heatmaps_add_PAFs false
#define heatmaps_scale 2
#define alpha_heatmap 0.7f
...
public:
	...
	std::unique_ptr<op::FaceDetector> faceDetector;
	//std::unique_ptr<op::FaceDetectorOpenCV> faceDetector;

	std::unique_ptr<op::PoseCpuRenderer> poseRenderer;
	std::unique_ptr<op::FaceCpuRenderer> faceRenderer;
	...
	std::vector<op::Rectangle<float>> FaceKey;
	...
	OpenPose(int FLAGS_logging_level = default_logging_level,
		...
		float FLAGS_face_render_threshold = default_face_render_threshold,
		...
		const auto netOutputSize = faceNetInputSize;
		// poseModel
		...
		const auto heatMapScale = op::flagsToHeatMapScaleMode(heatmaps_scale);
		...
		poseRenderer = std::unique_ptr<op::PoseCpuRenderer>(new op::PoseCpuRenderer{ poseModel, (float)FLAGS_render_threshold, !FLAGS_disable_blending, (float)FLAGS_alpha_pose });		
		...
		// Step4.1 - Initialize face
		faceDetector = std::unique_ptr<op::FaceDetector>(new op::FaceDetector(poseModel));
		//faceDetector = std::unique_ptr<op::FaceDetectorOpenCV>(new op::FaceDetectorOpenCV(FLAGS_model_folder));
		faceExtractorCaffe = std::unique_ptr<op::FaceExtractorCaffe>(new op::FaceExtractorCaffe{ faceNetInputSize, netOutputSize, FLAGS_model_folder, FLAGS_num_gpu_start, heatMapTypes, heatMapScale, enableGoogleLogging });
		faceRenderer = std::unique_ptr<op::FaceCpuRenderer>(new op::FaceCpuRenderer{ (float)FLAGS_face_render_threshold, (float)FLAGS_alpha_pose , alpha_heatmap });

		faceExtractorCaffe->initializationOnThread();
		faceRenderer->initializationOnThread();
	}

	std::vector<caffe::Blob<float>*> caffeNetSharedToPtr(std::vector<boost::shared_ptr<caffe::Blob<float>>>& caffeNetOutputBlob)
	{
		try
		{
	...
	void forward(const cv::Mat& inputImage, op::Array<float>& poseKeypoints, op::Array<float>& faceKeypoints, cv::Mat& displayImage, bool display = false) {
		...
		//printf("%d %d\n", (int)imageSize.x, (int)imageSize.y); 원본이미지 사이즈가 출력
		if (display) {
			auto outputArray = cvMatToOpOutput.createArray(inputImage, scaleInputToOutput, outputResolution);
			// Step 5 - Render poseKeypoints
			poseRenderer->renderPose(outputArray, poseKeypoints, scaleInputToOutput);
			//printf("%f", scaleInputToOutput);			
		...
		// Sttep 4.1 - Estimate Face Keypoints
		FaceKey = faceDetector->detectFaces(poseKeypoints);
		//FaceKey = faceDetector->detectFaces(inputImage);
		//for (auto i = 0; i< poseKeypoints.getSize(0); i++) ///for(int i =0; i<FaceKey.size();i++) //사람 숫자 나옴
		//	printf("Person %d : %f, %f , %f, %f\n", i + 1, FaceKey.at(i).x, FaceKey.at(i).y, FaceKey.at(i).width + FaceKey.at(i).x, FaceKey.at(i).height + FaceKey.at(i).y);

		faceExtractorCaffe->forwardPass(FaceKey, inputImage);
		//faceHeatMaps = faceExtractorCaffe->getHeatMaps() //히트맵 가져오기
		faceKeypoints = faceExtractorCaffe->getFaceKeypoints();

		if (display) {
			auto outputArray = cvMatToOpOutput.createArray(displayImage, scaleInputToOutput, outputResolution);
			faceRenderer->renderFace(outputArray, faceKeypoints, scaleInputToOutput);
			//faceRenderer->renderFaceInherited(outputArray, faceKeypoints);		
			displayImage = opOutputToCvMat.formatToCvMat(outputArray);
		}
	}
...
extern "C" {
    ...
    OP_EXPORT c_OP newOP(int logging_level,
		...
		float face_render_threshold,
		int num_gpu_start,
		bool disable_blending,
		char* model_folder
	) {
		return new OpenPose(logging_level, output_resolution, net_resolution, model_pose, alpha_pose,
			scale_gap, scale_number, render_threshold, face_render_threshold, num_gpu_start, disable_blending, model_folder);
	}
    OP_EXPORT void delOP(c_OP op) {
		delete (OpenPose *)op;
	}
    OP_EXPORT void forward(c_OP op, unsigned char* img, size_t rows, size_t cols, int* size, int* size2, unsigned char* displayImg, bool display) {
		OpenPose* openPose = (OpenPose*)op;
		cv::Mat image(rows, cols, CV_8UC3, img);
		cv::Mat displayImage(rows, cols, CV_8UC3, displayImg);
		openPose->forward(image, output, output2, displayImage, display);
		...
		if (output2.getSize().size()) {
			size2[0] = output2.getSize()[0];
			size2[1] = output2.getSize()[1];
			size2[2] = output2.getSize()[2];
		}
		else {
			size2[0] = 0; size2[1] = 0; size2[2] = 0;
		}
	...
	OP_EXPORT void getfaceOutputs(c_OP op, float* array) {
		if (output2.getSize().size()) {
			//printf("%d", output2.getSize());
			memcpy(array, output2.getPtr(), output2.getSize()[0] * output2.getSize()[1] * output2.getSize()[2] * sizeof(float));
		}
	}
