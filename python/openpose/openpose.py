...
class OpenPose(object):
    ...
    if platform == "linux" or platform == "linux2":
        ...
        try:
            _libop= np.ctypeslib.load_library('_openpose', 'D:/tensor/openpose/build/x64/Release/_openpose.dll')
        except OSError as e:
...
if __name__ == "__main__":
    ...
    display = False
    arr, arr2, output_image = openpose.forward(img, display)
    print("pose")
    #print(arr)
    for person in arr:
        for i in person:
            if(display):
                cv2.circle(output_image, (int(i[0]), int(i[1])), 3, (0,0,125), -1)  
            else :
                cv2.circle(img, (int(i[0]), int(i[1])), 3, (0,0,125), -1) 
    ...
    for person in arr2:
        for i in person:
            ...
            if(display):
                cv2.circle(output_image, (int(i[0]), int(i[1])), 1, (255,255,255), -1)               
            else :
                cv2.circle(img, (int(i[0]), int(i[1])), 1, (255,255,255), -1) 
    if(display):
